...
/* ── Avatar with platform badge + online dot ───────────────────────── */
  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      <div
        ...
        style={{ background: c.avatarBg, fontSize: size * 0.4 }}
      >
        {c.initial}
      </div>
      <span
        className="absolute -bottom-0.5 -right-0.5 flex items-center justify-center rounded-full border-2 border-card font-bold text-white"
        style={{ width: badge, height: badge, background: meta.bg, fontSize: badge * 0.5 }}
      >
        {meta.mark}
      </span>
      {c.online && (
        <span className="absolute -right-0 top-0 size-3 rounded-full border-2 border-card bg-success" />
      )}
    </div>
  );
}
        ...
        {filtered.map((c) => {
          ...
          return (
            <button
              ...
              className={cn(
                'flex w-full items-center gap-3 border-l-[3px] px-3.5 py-3 text-left transition-colors',
                active ? 'border-primary bg-secondary' : 'border-transparent hover:bg-muted',
              )}
            >
              <Avatar c={c} />
...
function ChatThread({ c, onSend }: { c: Conversation | null; onSend: (text: string) => void }) {
  ...
  if (!c) {
    return (
      <section className="flex flex-1 items-center justify-center bg-muted text-muted-foreground">
        เลือกบทสนทนาเพื่อเริ่มแชต
      </section>
    );
  }
    ...
    <section className="flex min-w-0 flex-1 flex-col bg-muted">
      <header className="flex items-center gap-3 border-b border-border bg-card px-4 py-2.5">
        ...
        <div>
          ...
          <div
            ...
            style={{ color: c.online ? '#10B981' : '#94A3B8' }}
          >
          ...
          >
            ...
            <div
              className={cn(
                ...
                m.from === 'shop' ? 'text-primary-foreground/70' : 'text-muted-foreground',
              )}
            >
              {m.time}
            </div>
      ...
      <div className="flex flex-wrap gap-1.5 border-t border-border bg-card px-3 pt-2.5">
        {quickReplies.map((q) => (
          <button
            key={q}
            onClick={() => setText(q)}
            className="rounded-full border border-border bg-muted px-2.5 py-1 text-xs text-muted-foreground transition-colors hover:bg-secondary hover:text-secondary-foreground"
          >
            {q}
          </button>
        ))}
      ...
      <form onSubmit={submit} className="flex items-center gap-2 bg-card p-3">
        <input
          ...
          className="h-10 flex-1 rounded-lg border border-input bg-background px-3.5 text-sm outline-none focus:ring-2 focus:ring-ring"
        />
        <Button type="submit" size="icon" aria-label="ส่ง">
          <Send />
        </Button>
      </form>
    </section>
...
function statusStyle(status: string): { bg: string; fg: string } {
  if (status.includes('จัดส่ง')) return { bg: '#ECFDF5', fg: '#059669' };
  if (status.includes('แพ็ก')) return { bg: '#EFF6FF', fg: '#2563EB' };
  if (status.includes('สลิป') || status.includes('ชำระ')) return { bg: '#FFFBEB', fg: '#D97706' };
      ...
      prev.map((c) =>
        c.id === selectedId
          ? {
              ...
              lastMessage: text,
              time: 'เดี๋ยวนี้',
              messages: [...c.messages, { id: `m${Date.now()}`, from: 'shop', text, time }],
            }
          : c,
      ),
    );
